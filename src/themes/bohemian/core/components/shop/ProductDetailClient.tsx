			...
			<>
				...
				<main className={`${bohemianBodyFont.className} bg-[#fcf9f4] pb-20 text-[#1c1c19]`}>
					<section className={`${BOHEMIAN_SITE_CONTAINER} pb-8 pt-8 md:pt-10`}>
						{/* Breadcrumb Skeleton */}
						<div className="w-48 h-4 mb-8 rounded shimmer-bg" />

						<div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-16">
							{/* Left Column Media Gallery Skeleton */}
							<div className="lg:col-span-7">
								<div className="flex flex-col gap-5 md:flex-row-reverse">
									<div className="relative aspect-[4/5] flex-1 rounded-lg bg-[#f6f3ee] shimmer-bg" />
									<div className="flex gap-3 md:w-24 md:flex-col">
										{Array.from({ length: 4 }).map((_, i) => (
											<div key={i} className="h-24 w-20 md:h-32 md:w-full rounded-lg shimmer-bg" />
										))}
									</div>
								</div>
							</div>
							...
							<div className="lg:col-span-5 lg:pt-2 flex flex-col gap-6">
								<div className="w-32 h-6 rounded-full shimmer-bg" />
								<div className="w-3/4 h-12 rounded shimmer-bg" />
								<div className="w-1/2 h-10 rounded shimmer-bg" />
								...
								<div className="flex gap-2">
									{Array.from({ length: 4 }).map((_, i) => (
										<div key={i} className="w-10 h-10 rounded-full shimmer-bg" />
									))}
								</div>
			...
			<main className={`${bohemianBodyFont.className} bg-[#fcf9f4] pb-20 text-[#1c1c19]`}>
						...
						<div className="lg:col-span-5 lg:pt-2">
							...
							<div className="mt-7 grid grid-cols-1 gap-3 sm:grid-cols-2">
								<button
									...
									className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#9f3f29] px-5 py-4 text-sm font-bold uppercase tracking-[0.08em] text-white transition hover:bg-[#bf573f] disabled:opacity-50 disabled:cursor-not-allowed"
								>
									{isAdding ? (
										<>
								...
								<button
									...
									disabled={isAdding || isBuying}
									className="inline-flex items-center justify-center rounded-lg border-2 border-[#9f3f29] px-5 py-4 text-sm font-bold uppercase tracking-[0.08em] text-[#9f3f29] transition hover:bg-[#9f3f29]/5 disabled:opacity-50 disabled:cursor-not-allowed"
								>
									{isBuying ? (
										<>
											<Loader2 className="h-4 w-4 animate-spin" />
					...
					<section className={`${BOHEMIAN_SITE_CONTAINER} mt-24`}>
						...
						<div className="mx-auto max-w-4xl divide-y divide-[#e8e4dc] border-t border-b border-[#e8e4dc]">
							{faqItems.map((faq: any, idx: number) => {
								const isExpanded = !!expandedAccordionItems[`faq-${idx}`];
								return (
									<div key={`faq-item-${idx}`} className="py-4">
										<button
											type="button"
											onClick={() => setExpandedAccordionItems(prev => ({
												...prev,
											...
											className="flex w-full items-center justify-between text-left focus:outline-none py-2"
										>
											<span className="font-semibold text-lg text-[#1c1c19]">{faq.question || faq.q}</span>
											<span className="ml-6 flex-shrink-0 text-[#9f3f29]">
												{isExpanded ? (
													<Minus className="h-5 w-5" />
												) : (
										...
										{isExpanded && (
											<div className="mt-2 pr-12 text-base leading-relaxed text-[#56423d] whitespace-pre-wrap animate-fade-in">
												{faq.answer || faq.a}
											</div>
										)}
									</div>
				...
				<section className={`${BOHEMIAN_SITE_CONTAINER} mt-24`}>
					...
					{relatedProducts.length > 0 ? (
						<div className="flex gap-7 overflow-x-auto pb-6 -mx-4 px-4 sm:mx-0 sm:px-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] snap-x snap-mandatory">
							{relatedProducts.map((related) => (
								<article key={related.id} className="group w-[265px] flex-shrink-0 snap-start">
									<Link href={`/shop/${related.slug}`} className="block">
										<div className="relative mb-4 aspect-[3/4] overflow-hidden rounded-lg bg-[#f0ede8]">
											{related.image ? (
												<Image
													src={getThumbnailUrl(related.image)}
													alt={related.name}
													fill
